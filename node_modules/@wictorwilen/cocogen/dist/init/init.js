...
import { COCOGEN_CONFIG_FILE, loadProjectConfig, projectConfigContents, } from "./project-config.js";
import { buildSampleCsv, buildSampleJson, buildSamplePersonEntityPayload, buildSampleYaml, exampleValueForPayload, samplePayloadValueForType, } from "./sample-data.js";
...
function buildRestConnectionPayload(ir) {
    const payload = {
...
function buildRestItemPayload(ir, itemId) {
    const properties = {};
    for (const prop of ir.properties) {
        if (prop.name === ir.item.contentPropertyName)
            continue;
        ...
        if (structuredFields && prop.type !== "principal" && prop.type !== "principalCollection") {
            ...
            const value = exampleValue ??
                samplePayloadValueForType(prop.type, prop.personEntity ? prop.personEntity.fields : null, prop.source);
...
async function writeRestFiles(outDir, ir) {
    ...
    const itemId = typeof idExample === "string"
        ? idExample
    ...
    await writeFile(path.join(outDir, "create-connection.http"), await renderTemplate("rest/create-connection.http.ejs", {
        graphBaseUrl: graphBaseUrlForOperation(ir, "connectionProvisioning"),
        connectionId,
        connectionPayloadJson,
    }), "utf8");
    await writeFile(path.join(outDir, "patch-schema.http"), await renderTemplate("rest/patch-schema.http.ejs", {
        graphBaseUrl: graphBaseUrlForOperation(ir, "schemaRegistration"),
        connectionId,
        schemaPayloadJson,
    }), "utf8");
    await writeFile(path.join(outDir, "ingest-item.http"), await renderTemplate("rest/ingest-item.http.ejs", {
    ...
    if (ir.connection.contentCategory === "people") {
        ...
        const profileSourceDisplayName = ir.connection.profileSource?.displayName ?? ir.connection.connectionName ?? ir.item.typeName;
        const profileSourcePriority = ir.connection.profileSource?.priority ?? "first";
        await writeFile(path.join(outDir, "profile-source.http"), await renderTemplate("rest/profile-source.http.ejs", {
...
function throwIfPreviewFeaturesRequired(ir, allowPreviewFeatures) {
...
export async function updateTsProject(options) {
    ...
    if (options.tspPath) {
        await writeFile(path.join(outDir, COCOGEN_CONFIG_FILE), projectConfigContents(outDir, tspPath, "ts", config.inputFormat), "utf8");
    }
    return { outDir, ir };
}
...
export async function updateDotnetProject(options) {
    ...
    await generator.writeGenerated();
    if (options.tspPath) {
        await writeFile(path.join(outDir, COCOGEN_CONFIG_FILE), projectConfigContents(outDir, tspPath, "dotnet", config.inputFormat), "utf8");
    }
    return { outDir, ir };
}
/** Update an existing REST project from a schema. */
export async function updateRestProject(options) {
    const outDir = path.resolve(options.outDir);
    const { config } = await loadProjectConfig(outDir);
    const tspPath = options.tspPath ? path.resolve(options.tspPath) : path.resolve(outDir, config.tsp);
    if (config.lang !== "rest") {
    ...
    if (options.tspPath) {
        await writeFile(path.join(outDir, COCOGEN_CONFIG_FILE), projectConfigContents(outDir, tspPath, "rest", config.inputFormat), "utf8");
    }
    return { outDir, ir };
}
...
export async function initRestProject(options) {
    const outDir = path.resolve(options.outDir);
    ...
    await writeFile(path.join(outDir, COCOGEN_CONFIG_FILE), projectConfigContents(outDir, copiedTspPath, "rest", ir.connection.inputFormat), "utf8");
    await writeRestFiles(outDir, ir);
    return { outDir, ir };
}
