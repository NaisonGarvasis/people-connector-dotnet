...
function jsonPathToSteps(path) {
    ...
    const parseBracketContent = (rawContent) => {
...
function setNestedValue(target, steps, value) {
    if (steps.length === 0)
        return;
    let cursor = target;
...
function setNestedObject(target, segments, value) {
...
function sampleValueForPrincipalKey(key, index = 0) {
...
function buildSamplePrincipalObject(fields, fallbackSource, index = 0) {
...
function buildSamplePrincipalCollection(fields, fallbackSource) {
    return [
...
function buildSamplePersonEntityObject(fields, index = 0) {
...
function buildSamplePersonEntityPayload(fields, isCollection) {
    if (!isCollection) {
...
function exampleValueForPayload(example, type) {
    if (example === undefined || example === null)
        return undefined;
    if (type.endsWith("Collection")) {
        if (Array.isArray(example))
...
function sampleJsonValueForType(type) {
    switch (type) {
        case "boolean":
            return true;
        case "int64":
...
function buildSerializedSampleObject(model) {
    const result = {};
...
function buildSerializedSampleValue(model, type) {
...
function samplePayloadValueForType(type, fields, fallbackSource) {
    switch (type) {
        case "boolean":
            return true;
        case "int64":
...
function buildPrincipalFieldEntries(fields, fallbackSource) {
        ...
        if (structuredFields) {
            ...
            for (const field of structuredFields) {
                ...
                if (path.includes("[*]")) {
                    ...
                    if (!arrayGroups.has(root))
                        arrayGroups.set(root, []);
...
function buildSampleCsv(ir) {
    ...
    for (const prop of ir.properties) {
        ...
        if (structuredFields) {
            for (const field of structuredFields)
                addHeader(field.source.csvHeaders[0] ?? field.path);
            continue;
        }
...
function buildSampleJson(ir) {
    return JSON.stringify([buildSampleItem(ir)], null, 2) + "\n";
}
/** Build a YAML sample document for a connector IR. */
function buildSampleYaml(ir) {
    const yaml = stringifyYaml([buildSampleItem(ir)]);
    return yaml.endsWith("\n") ? yaml : `${yaml}\n`;
}
export { buildSampleCsv, buildSampleJson, buildSampleYaml, buildSamplePersonEntityPayload, exampleValueForPayload, exampleValueForType, samplePayloadValueForType, };
