...
export function buildPeopleGraphTypes(ir) {
    ...
    const resolveReferencedGraphType = (propType) => {
        ...
        if (GRAPH_STRING_TYPES.has(elementType))
            return null;
        if (elementType.startsWith("Edm."))
            return null;
        const graphName = resolveGraphTypeName(elementType);
        if (graphName && isProfileEnum(graphName))
            return null;
    ...
    while (addedReference) {
        ...
        for (const name of [...graphTypeNames]) {
            ...
            for (const prop of schemaType.properties ?? []) {
                ...
                if (!referenced)
                    continue;
                if (graphTypeNames.has(referenced))
                    continue;
    ...
    const templates = [...graphTypeNames].map((typeName) => {
        ...
        const fields = (schemaType.properties ?? []).map((prop) => convertGraphProperty(prop, usedVarNames, aliases));
        return {
...
export function buildPeopleLabelSerializers() {
...
export function buildGraphEnumTemplates() {
...
function buildDerivedPeopleGraphTypes(ir) {
    ...
    const buildDerivedFromTree = (typeName, node) => {
            ...
            for (const [key, value] of Object.entries(node)) {
                if (existingNames.has(key)) {
                    if (typeof value === "object" && value && !("path" in value)) {
                        ...
                        buildDerivedFromTree(nestedName, value);
                    }
                    continue;
                }
                ...
                if (typeof value === "object" && value && "path" in value) {
                    ...
                    const nestedName = `${typeName}${toCsPascal(key)}`;
                    const nested = buildDerivedFromTree(nestedName, value);
                    existing.fields.push({
                        name: key,
                        varName,
                        tsType: nested.alias,
    ...
    for (const [entity, fields] of fieldsByEntity) {
        ...
        for (const prop of schemaType.properties ?? []) {
            ...
            if (!graphName || isCollection || GRAPH_STRING_TYPES.has(propType) || isProfileEnum(graphName))
                continue;
            if (schemaTypes.has(graphName))
                continue;
            if (!node || typeof node !== "object")
                continue;
            buildDerivedFromTree(graphName, node);
        }
    }
    const referencedGraphTypes = new Set();
    for (const schemaType of graphProfileSchema.types) {
        for (const prop of schemaType.properties ?? []) {
            ...
            if (!graphName)
                continue;
            if (isProfileEnum(graphName))
                continue;
            if (schemaTypes.has(graphName))
                continue;
            if (derived.has(graphName))
                continue;
...
function buildPeopleGraphTypeAliases(derived) {
    const map = new Map();
...
function convertGraphProperty(prop, usedVarNames, graphAliases) {
...
export function parseGraphTypeDescriptor(typeName, graphAliases) {
...
function getScalarDescriptor(typeName) {
    switch (typeName) {
        case "Edm.String":
        case "Edm.Date":
...
function toPeopleFieldVarName(name, used) {
