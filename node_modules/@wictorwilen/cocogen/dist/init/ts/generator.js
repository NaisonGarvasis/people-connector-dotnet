...
import { toTsIdentifier, toTsSchemaFolderName, toTsType, } from "../naming.js";
...
import { buildGraphEnumTemplates, buildPeopleGraphTypes, buildPeopleLabelSerializers, parseGraphTypeDescriptor, } from "../people/graph-types.js";
import { CoreGenerator } from "../generators/core.js";
import { removeIfExists } from "../helpers/fs.js";
import { buildTsPersonEntityCollectionExpression, buildTsPersonEntityExpression } from "./people-entity.js";
import { buildTsPrincipalCollectionExpression, buildTsPrincipalExpression } from "./principal.js";
import { applyTsValidationExpression, buildTsStringConstraintsLiteral, buildTsValidationCall, } from "./validation.js";
/** Generates TypeScript connector scaffolds and generated files. */
export class TsGenerator extends CoreGenerator {
    lang = "ts";
    constructor(context) {
        super(context);
    }
    ...
    async writeScaffold() {
        ...
        await writeFile(path.join(this.outDir, "package.json"), await renderTemplate("ts/package.json.ejs", {
        ...
        await writeFile(path.join(this.outDir, "tspconfig.yaml"), await renderTemplate("ts/tspconfig.yaml.ejs", {}), "utf8");
        await writeFile(path.join(this.outDir, "tsconfig.json"), await renderTemplate("ts/tsconfig.json.ejs", {}), "utf8");
        await writeFile(path.join(this.outDir, ".env.example"), await renderTemplate("ts/.env.example.ejs", {
        ...
        await writeFile(path.join(this.outDir, "README.md"), await renderTemplate("ts/README.md.ejs", {
            isPeopleConnector: this.ir.connection.contentCategory === "people",
            itemTypeName: this.ir.item.typeName,
            schemaFolderName,
            inputFormat: this.ir.connection.inputFormat,
        ...
        await writeFile(path.join(this.outDir, "src", "cli.ts"), await renderTemplate("ts/src/cli.ts.ejs", {
        ...
        await writeFile(path.join(this.outDir, "src", "datasource", "itemSource.ts"), await renderTemplate("ts/src/datasource/itemSource.ts.ejs", {
            itemTypeName: this.ir.item.typeName,
            schemaFolderName,
        }), "utf8");
        if (this.ir.connection.inputFormat === "json") {
            await writeFile(path.join(this.outDir, "src", "datasource", "jsonItemSource.ts"), await renderTemplate("ts/src/datasource/jsonItemSource.ts.ejs", {
                itemTypeName: this.ir.item.typeName,
                schemaFolderName,
            }), "utf8");
        }
        else if (this.ir.connection.inputFormat === "yaml") {
            await writeFile(path.join(this.outDir, "src", "datasource", "yamlItemSource.ts"), await renderTemplate("ts/src/datasource/yamlItemSource.ts.ejs", {
                itemTypeName: this.ir.item.typeName,
                schemaFolderName,
            }), "utf8");
        }
        else if (this.ir.connection.inputFormat === "rest") {
            await writeFile(path.join(this.outDir, "src", "datasource", "restItemSource.ts"), await renderTemplate("ts/src/datasource/restItemSource.ts.ejs", {
                itemTypeName: this.ir.item.typeName,
                schemaFolderName,
            }), "utf8");
        }
        else if (this.ir.connection.inputFormat === "custom") {
            await writeFile(path.join(this.outDir, "src", "datasource", "customItemSource.ts"), await renderTemplate("ts/src/datasource/customItemSource.ts.ejs", {
                itemTypeName: this.ir.item.typeName,
                schemaFolderName,
            }), "utf8");
        }
        else {
            await writeFile(path.join(this.outDir, "src", "datasource", "csvItemSource.ts"), await renderTemplate("ts/src/datasource/csvItemSource.ts.ejs", {
                itemTypeName: this.ir.item.typeName,
                schemaFolderName,
            }), "utf8");
        }
        await writeFile(path.join(this.outDir, "src", "index.ts"), await renderTemplate("ts/src/index.ts.ejs", { schemaFolderName }), "utf8");
    }
    ...
    async writeGenerated() {
        ...
        const hasPeopleSupport = this.ir.connection.contentCategory === "people" ||
        ...
        const graphAliases = peopleGraphTypesBundle ? peopleGraphTypesBundle.aliases : new Map();
        const peopleGraphTypeByAlias = new Map(peopleGraphTypes.map((type) => [type.alias, type]));
        const peopleProfileTypeInfoByAlias = new Map();
        const buildPeopleTypeImports = (aliases) => {
            const local = new Set();
        ...
        if (hasPeopleSupport && peopleGraphTypesBundle) {
            ...
            for (const type of peopleGraphTypesBundle.derived) {
                ...
                const properties = new Map(type.fields.map((field) => [field.name, field.tsType]));
                peopleProfileTypeInfoByAlias.set(alias, { alias, properties });
            }
        }
        const collectPeopleEntityTypes = (fields, typeInfo) => {
            const used = new Set();
            if (!typeInfo)
                return used;
            const tree = buildObjectTree(fields);
            const visit = (node, info) => {
                if (!info)
                    return;
                used.add(info.alias);
                for (const [key, value] of Object.entries(node)) {
                    if (typeof value === "object" && value && !Array.isArray(value) && !("path" in value)) {
        ...
        const peopleEntityTypes = new Set();
        const transformProperties = this.ir.properties.map((p) => {
            ...
            const applySourceStringExpression = (expr) => {
            ...
            const applySourceCollectionExpression = (expr) => {
            ...
            if (personEntityTypeInfo) {
                for (const typeName of collectPeopleEntityTypes(p.personEntity?.fields.map((field) => ({ path: field.path, source: field.source })) ?? [], personEntityTypeInfo)) {
                ...
                ? (p.type === "stringCollection"
                    ? buildTsPersonEntityCollectionExpression(p.personEntity.fields.map((field) => ({
                        ...
                        return stringConstraints
                            ...
                            : base;
                    }, personEntityTypeInfo, peopleProfileTypeInfoByAlias)
            ...
            const mappedObject = p.mappedObject
                ? (p.type === "stringCollection"
                    ? buildTsPersonEntityCollectionExpression(p.mappedObject.fields.map((field) => ({
                        path: field.path,
                        source: field.source,
                    })), (headersLiteral) => {
                        const base = `parseStringCollection(readSourceValue(row, ${headersLiteral}))`;
                    ...
                    : buildTsPersonEntityExpression(p.mappedObject.fields.map((field) => ({
                        path: field.path,
                        source: field.source,
                    })), (headersLiteral) => {
                        const base = `parseString(readSourceValue(row, ${headersLiteral}))`;
        ...
        const idRawExpression = idRawSource
            ...
            : '""';
        const usesPrincipal = this.ir.properties.some((p) => p.type === "principal" || p.type === "principalCollection");
        ...
        const payloadPeopleGraphTypes = new Set();
        const payloadProperties = this.ir.properties
            ...
            const baseValue = p.type === "principal"
        ...
        const collectPeopleHelperGraphTypes = (directAliases) => {
        ...
        const peopleHelperGraphTypes = collectPeopleHelperGraphTypes(new Set([...peopleEntityTypeImports.local, ...payloadPeopleTypeImports.local]));
        await writeFile(path.join(this.outDir, "src", schemaFolderName, "model.ts"), await renderTemplate("ts/src/generated/model.ts.ejs", {
            itemTypeName: this.ir.item.typeName,
            properties: modelProperties,
            itemDocComment: this.ir.item.doc ? formatDocComment(this.ir.item.doc) : undefined,
            usesPrincipal,
        }), "utf8");
        await writeFile(path.join(this.outDir, "src", "datasource", "row.ts"), await renderTemplate("ts/src/generated/row.ts.ejs", {
            inputFormat: this.ir.connection.inputFormat,
        }), "utf8");
        if (this.ir.connection.inputFormat !== "csv") {
            await writeFile(path.join(this.outDir, "src", "datasource", "inputPath.ts"), await renderTemplate("ts/src/datasource/inputPath.ts.ejs", {}), "utf8");
        }
        await writeFile(path.join(this.outDir, "src", schemaFolderName, "constants.ts"), await renderTemplate("ts/src/generated/constants.ts.ejs", {
        ...
        await writeFile(path.join(this.outDir, "src", schemaFolderName, "schemaPayload.ts"), await renderTemplate("ts/src/generated/schemaPayload.ts.ejs", {
            schemaPayloadJson: JSON.stringify(schemaPayload(this.ir), null, 2),
        }), "utf8");
        if (hasPeopleSupport) {
            await writeFile(path.join(this.outDir, "src", "core", "people.ts"), await renderTemplate("ts/src/core/people.ts.ejs", {
                graphTypes: peopleHelperGraphTypes,
                labels: peopleLabelSerializers,
                graphEnums: buildGraphEnumTemplates(),
            }), "utf8");
        }
        await writeFile(path.join(this.outDir, "src", schemaFolderName, "propertyTransformBase.ts"), await renderTemplate("ts/src/generated/propertyTransformBase.ts.ejs", {
        ...
        await writeFile(path.join(this.outDir, "src", schemaFolderName, "fromRow.ts"), await renderTemplate("ts/src/generated/fromRow.ts.ejs", {
            properties: transformProperties,
            itemTypeName: this.ir.item.typeName,
            idRawExpression,
            usesPrincipal,
        ...
        const contentValueExpression = this.ir.item.contentPropertyName
            ...
            : "\"\"";
        await writeFile(path.join(this.outDir, "src", schemaFolderName, "itemPayload.ts"), await renderTemplate("ts/src/generated/itemPayload.ts.ejs", {
        ...
        await writeFile(path.join(this.outDir, "src", schemaFolderName, "index.ts"), await renderTemplate("ts/src/generated/index.ts.ejs", {}), "utf8");
        await writeFile(path.join(this.outDir, "src", "core", "connectorCore.ts"), await renderTemplate("ts/src/core/connectorCore.ts.ejs", {
            itemTypeName: this.ir.item.typeName,
            isPeopleConnector: this.ir.connection.contentCategory === "people",
        }), "utf8");
        await writeFile(path.join(this.outDir, "src", "core", "validation.ts"), await renderTemplate("ts/src/core/validation.ts.ejs", {}), "utf8");
        await writeFile(path.join(this.outDir, "src", "core", "itemId.ts"), await renderTemplate("ts/src/core/itemId.ts.ejs", {}), "utf8");
        if (usesPrincipal) {
            await writeFile(path.join(this.outDir, "src", "core", "principal.ts"), await renderTemplate("ts/src/core/principal.ts.ejs", {}), "utf8");
        }
    }
}
