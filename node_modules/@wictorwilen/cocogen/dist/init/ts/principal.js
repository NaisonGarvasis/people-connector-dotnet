...
export function buildTsPrincipalExpression(fields, fallbackSource) {
    const entries = buildPrincipalFieldEntries(fields, fallbackSource).map((entry) => {
...
export function buildTsPrincipalCollectionExpression(fields, fallbackSource) {
    const entries = buildPrincipalFieldEntries(fields, fallbackSource);
    if (entries.length === 0)
        return "[]";
    const fieldLines = entries.map((entry, index) => `  const field${index} = ${applySourceCollectionExpression(`parseStringCollection(readSourceValue(row, ${buildSourceLiteral(entry.source)}))`, entry.source)};`);
