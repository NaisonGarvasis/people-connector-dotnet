...
import { toCsIdentifier, toCsNamespace, toCsPascal, toCsPropertyName, toCsType, toSchemaFolderName, toTsIdentifier, } from "../naming.js";
...
import { graphApiVersionForOperation, toCsParseFunction, toCsPropertyValueExpression, toGraphPropertyTypeEnumName, toOdataCollectionType, } from "../helpers/schema.js";
import { GRAPH_STRING_TYPES, buildGraphEnumTemplates, buildPeopleGraphTypes, resolveGraphTypeName, } from "../people/graph-types.js";
import { CoreGenerator } from "../generators/core.js";
import { buildCsPrincipalCollectionExpression, buildCsPrincipalExpression } from "./principal.js";
import { buildCsPersonEntityCollectionExpression, buildCsPersonEntityExpression, } from "./people-entity.js";
import { applyCsValidationExpression, buildCsStringConstraintsLiteral } from "./validation.js";
function formatCsTransformExpression(expression, continuationIndent) {
...
export class DotnetGenerator extends CoreGenerator {
    lang = "dotnet";
    constructor(context) {
        super(context);
    }
    ...
    async writeScaffold() {
        ...
        await writeFile(path.join(this.outDir, `${this.settings.projectName}.csproj`), await renderTemplate("dotnet/project.csproj.ejs", {
        ...
        await writeFile(path.join(this.outDir, "package.json"), await renderTemplate("dotnet/package.json.ejs", {
            projectName: this.settings.projectName,
        }), "utf8");
        await writeFile(path.join(this.outDir, "tspconfig.yaml"), await renderTemplate("dotnet/tspconfig.yaml.ejs", {}), "utf8");
        await this.writeProgramFile();
        await writeFile(path.join(this.outDir, "Datasource", "IItemSource.cs"), await renderTemplate("dotnet/Datasource/IItemSource.cs.ejs", {
            namespaceName,
            schemaNamespace,
            itemTypeName: this.ir.item.typeName,
        }), "utf8");
        if (this.ir.connection.inputFormat === "json") {
            await writeFile(path.join(this.outDir, "Datasource", "JsonItemSource.cs"), await renderTemplate("dotnet/Datasource/JsonItemSource.cs.ejs", {
                namespaceName,
                schemaNamespace,
                itemTypeName: this.ir.item.typeName,
            }), "utf8");
            ...
            await writeFile(path.join(this.outDir, "Datasource", "YamlItemSource.cs"), await renderTemplate("dotnet/Datasource/YamlItemSource.cs.ejs", {
                namespaceName,
                schemaNamespace,
                itemTypeName: this.ir.item.typeName,
            }), "utf8");
            ...
            await writeFile(path.join(this.outDir, "Datasource", "RestItemSource.cs"), await renderTemplate("dotnet/Datasource/RestItemSource.cs.ejs", {
                namespaceName,
                schemaNamespace,
                itemTypeName: this.ir.item.typeName,
            }), "utf8");
            ...
            await writeFile(path.join(this.outDir, "Datasource", "CustomItemSource.cs"), await renderTemplate("dotnet/Datasource/CustomItemSource.cs.ejs", {
                namespaceName,
                schemaNamespace,
                itemTypeName: this.ir.item.typeName,
            }), "utf8");
            ...
            await writeFile(path.join(this.outDir, "Datasource", "CsvItemSource.cs"), await renderTemplate("dotnet/Datasource/CsvItemSource.cs.ejs", {
                namespaceName,
                schemaNamespace,
                itemTypeName: this.ir.item.typeName,
            }), "utf8");
        }
        await writeFile(path.join(this.outDir, "appsettings.json"), await renderTemplate("dotnet/appsettings.json.ejs", {
            itemTypeName: this.ir.item.typeName,
            isPeopleConnector: this.ir.connection.contentCategory === "people",
            inputFormat: this.ir.connection.inputFormat,
        ...
        await writeFile(path.join(this.outDir, ".gitignore"), await renderTemplate("dotnet/.gitignore.ejs", {}), "utf8");
        await writeFile(path.join(this.outDir, "README.md"), await renderTemplate("dotnet/README.md.ejs", {
            isPeopleConnector: this.ir.connection.contentCategory === "people",
            itemTypeName: this.ir.item.typeName,
            schemaFolderName,
            inputFormat: this.ir.connection.inputFormat,
    ...
    async writeProgramFile() {
        const { namespaceName, schemaNamespace } = this;
        await writeFile(path.join(this.outDir, "Program.cs"), await renderTemplate("dotnet/Program.commandline.cs.ejs", {
            namespaceName,
            schemaNamespace,
            itemTypeName: this.ir.item.typeName,
            isPeopleConnector: this.ir.connection.contentCategory === "people",
    ...
    async writeGenerated() {
        ...
        const qualifyGraphModelType = (csName, sourcePackage) => sourcePackage === "v1" ? `Microsoft.Graph.Models.${csName}` : `Microsoft.Graph.Beta.Models.${csName}`;
        const resolveGraphModelCsType = (graphName) => {
        ...
        const resolveGraphEnumCsType = (enumName) => qualifyGraphModelType(toCsPascal(enumName), "beta");
        const resolveCsType = (rawType) => {
        ...
        const peopleProfileTypes = [...baseProfileTypes, ...derivedProfileTypes].sort((a, b) => {
            if (a.csName === "ItemFacet")
                return -1;
            if (b.csName === "ItemFacet")
                return 1;
            return a.csName.localeCompare(b.csName);
        });
        const baseTypeNames = new Set(peopleProfileTypes
            .filter((type) => !type.sourcePackage)
            .map((type) => type.baseType)
            .filter((name) => Boolean(name)));
        const schemaBaseTypeByName = new Map(graphProfileSchema.types.map((type) => [type.name, type.baseType]));
        const isItemFacetType = (typeName) => {
            if (typeName === "itemFacet")
                return true;
            let current = schemaBaseTypeByName.get(typeName);
            while (current) {
                if (current === "itemFacet")
                    return true;
                current = schemaBaseTypeByName.get(current);
            }
            return false;
        };
        const peopleProfileTypeInfoByName = new Map(peopleProfileTypes.map((type) => [
            type.csName,
            {
        ...
        const peopleProfileTypeByName = new Map(peopleProfileTypes.map((type) => [type.name, type]));
        ...
        const properties = this.ir.properties.map((p) => {
            ...
            const personEntityTypeInfo = personEntityType
                ? {
                    ...
                    properties: new Map(personEntityType.properties.map((prop) => [prop.name, { csName: prop.csName, csType: prop.csType }])),
                }
                : null;
                    ...
                    : contentExpression
                        ...
                        : (p.type === "principal" || p.type === "principalCollection") && principalExpression
                            ...
                            : personEntity
                                ? isCollection
                                    ? buildCsPersonEntityCollectionExpression(personEntity.fields, (headersLiteral) => {
                                        ...
                                        return csStringConstraints.hasAny
                                            ...
                                            : base;
                                    }, personEntityTypeInfo, peopleProfileTypeInfoByName, this.ir.connection.inputFormat)
                                ...
                                : mappedObject
                                    ? isCollection
                                        ? buildCsPersonEntityCollectionExpression(mappedObject.fields, (headersLiteral) => {
                                            const base = `RowParser.ParseStringCollection(row, ${headersLiteral})`;
                                            return csStringConstraints.hasAny
                                                ? `Validation.ValidateStringCollection(${nameLiteral}, ${base}, ${csStringConstraints.minLength}, ${csStringConstraints.maxLength}, ${csStringConstraints.pattern}, ${csStringConstraints.format})`
                                                : base;
                                        }, null, new Map(), this.ir.connection.inputFormat)
                                        : buildCsPersonEntityExpression(mappedObject.fields, (headersLiteral) => {
                                            const base = `RowParser.ParseString(row, ${headersLiteral})`;
                                            return csStringConstraints.hasAny
                                                ? `Validation.ValidateString(${nameLiteral}, ${base}, ${csStringConstraints.minLength}, ${csStringConstraints.maxLength}, ${csStringConstraints.pattern}, ${csStringConstraints.format})`
                                                : base;
        ...
        const schemaPropertyLines = properties
            ...
            if (isPrincipalType)
                additionalDataEntries.push(`                        ["type"] = ${JSON.stringify(p.type)},`);
        ...
        const idRawSourceDotnet = itemIdProperty?.personEntity?.fields[0]?.source ?? itemIdProperty?.source;
        ...
        const propertyInitializerLines = [
            ...properties.map((p) => `            ${p.csName} = transforms.TransformProperty<${p.csType}>(${JSON.stringify(p.name)}, row),`),
        ...
        await writeFile(path.join(this.outDir, schemaFolderName, "Model.cs"), await renderTemplate("dotnet/Generated/Model.cs.ejs", {
            namespaceName,
            schemaNamespace,
            itemTypeName: this.ir.item.typeName,
        ...
        await writeFile(path.join(this.outDir, schemaFolderName, "Constants.cs"), await renderTemplate("dotnet/Generated/Constants.cs.ejs", {
            schemaNamespace,
            graphApiVersion: this.ir.connection.graphApiVersion,
        ...
        await writeFile(path.join(this.outDir, schemaFolderName, "SchemaPayload.cs"), await renderTemplate("dotnet/Generated/SchemaPayload.cs.ejs", {
            schemaNamespace,
            schemaPropertyLines,
            graphApiVersion: this.ir.connection.graphApiVersion,
        }), "utf8");
        await writeFile(path.join(this.outDir, "Datasource", "RowParser.cs"), await renderTemplate("dotnet/Generated/RowParser.cs.ejs", {
            namespaceName,
            inputFormat: this.ir.connection.inputFormat,
        }), "utf8");
        await writeFile(path.join(this.outDir, schemaFolderName, "PropertyTransformBase.cs"), await renderTemplate("dotnet/Generated/PropertyTransformBase.cs.ejs", {
            namespaceName,
            schemaNamespace,
            properties,
        ...
        await writeFile(path.join(this.outDir, schemaFolderName, "FromRow.cs"), await renderTemplate("dotnet/Generated/FromRow.cs.ejs", {
            namespaceName,
            schemaNamespace,
            itemTypeName: this.ir.item.typeName,
        ...
        await writeFile(path.join(this.outDir, schemaFolderName, "ItemPayload.cs"), await renderTemplate("dotnet/Generated/ItemPayload.cs.ejs", {
            namespaceName,
            schemaNamespace,
            itemTypeName: this.ir.item.typeName,
            itemIdExpression,
        ...
        await writeFile(path.join(this.outDir, "Core", "ConnectorCore.cs"), await renderTemplate("dotnet/Core/ConnectorCore.cs.ejs", {
            namespaceName,
            schemaNamespace,
            itemTypeName: this.ir.item.typeName,
            isPeopleConnector: this.ir.connection.contentCategory === "people",
        ...
        await writeFile(path.join(this.outDir, "Core", "IItemPayload.cs"), await renderTemplate("dotnet/Core/IItemPayload.cs.ejs", {
            namespaceName,
            graphApiVersion: this.ir.connection.graphApiVersion,
        }), "utf8");
        await writeFile(path.join(this.outDir, "Core", "Validation.cs"), await renderTemplate("dotnet/Core/Validation.cs.ejs", {
            namespaceName,
        }), "utf8");
        if (usesPeopleLabels) {
            await writeFile(path.join(this.outDir, "Core", "PeoplePayload.cs"), await renderTemplate("dotnet/Core/PeoplePayload.cs.ejs", {
                namespaceName,
        ...
        await writeFile(path.join(this.outDir, "Core", "ItemId.cs"), await renderTemplate("dotnet/Core/ItemId.cs.ejs", {
            namespaceName,
        }), "utf8");
        if (usesPrincipal) {
            await writeFile(path.join(this.outDir, "Core", "Principal.cs"), await renderTemplate("dotnet/Core/Principal.cs.ejs", {
                namespaceName,
            }), "utf8");
        }
    }
