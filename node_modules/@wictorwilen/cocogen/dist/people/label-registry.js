...
import { getProfilePlanTypeNameByLabel, getProfileType, resolveProfileTypeName } from "./profile-schema.js";
const RAW_LABELS = [
...
export function supportedPeopleLabels() {
    return RAW_LABELS.map((label) => label.label);
}
export function isSupportedPeopleLabel(label) {
    return SUPPORTED_PEOPLE_LABELS.has(label);
}
export function getPeopleLabelDefinition(label) {
    return labelDefinitions.get(label);
}
export const BLOCKED_PEOPLE_LABELS = new Map([
    [
        "personManager",
        {
...
export const getBlockedPeopleLabel = (label) => BLOCKED_PEOPLE_LABELS.get(label);
export const getPeopleLabelInfo = (label) => {
...
//# sourceMappingURL=label-registry.js.map