...
import { NodeHost, compile, formatDiagnostic, getDoc, getExamples, serializeValueAsJson, getFormat, getPatternData, getDeprecated, getMinLengthAsNumeric, getMaxLengthAsNumeric, getMinValueAsNumeric, getMaxValueAsNumeric, isArrayModelType, } from "@typespec/compiler";
import { getConnectionPropertyCapability, getPropertyTypeCapability, maxGraphApiVersion, } from "../graph/capabilities.js";
import { getGraphOperationRequirements } from "../graph/requirements.js";
import { getPeopleLabelDefinition } from "../people/label-registry.js";
...
import { COCOGEN_STATE_CONNECTION_SETTINGS, COCOGEN_STATE_PROFILE_SOURCE_SETTINGS, COCOGEN_STATE_CONTENT_PROPERTIES, COCOGEN_STATE_CONTENT_SETTINGS, COCOGEN_STATE_ID_PROPERTIES, COCOGEN_STATE_ITEM_MODELS, COCOGEN_STATE_PROPERTY_ALIASES, COCOGEN_STATE_PROPERTY_SCHEMA_DESCRIPTIONS, COCOGEN_STATE_PROPERTY_LEGACY_DESCRIPTIONS, COCOGEN_STATE_PROPERTY_TYPESPEC_DESCRIPTIONS, COCOGEN_STATE_PROPERTY_LABELS, COCOGEN_STATE_PROPERTY_NAME_OVERRIDES, COCOGEN_STATE_PROPERTY_SEARCH, COCOGEN_STATE_PROPERTY_SOURCE, COCOGEN_STATE_PROPERTY_SOURCE_ENTRIES, COCOGEN_STATE_PROPERTY_SOURCE_DEFAULT, COCOGEN_STATE_PROPERTY_SOURCE_TRANSFORMS, COCOGEN_STATE_PROPERTY_NO_SOURCE, COCOGEN_STATE_PROPERTY_PERSON_FIELDS, COCOGEN_STATE_PROPERTY_SERIALIZED, COCOGEN_STATE_ID_SETTINGS, } from "../typespec/state.js";
const PEOPLE_ENTITY_PROPERTY_ALIASES = new Map([
    ["physicalAddress", new Map([["country", "countryOrRegion"]])],
]);
function normalizePersonEntityGraphPath(entity, rawPath) {
...
function requireSingleItemModel(program) {
    const items = [...program.stateSet(COCOGEN_STATE_ITEM_MODELS)].filter((t) => t.kind === "Model");
    if (items.length === 0) {
        throw new CocogenError("No @coco.item() model found. Add @coco.item() to the model that represents an external item.");
    }
    if (items.length > 1) {
        throw new CocogenError(`Multiple @coco.item() models found (${items
            .map((m) => m.name)
            .join(", ")}). Use exactly one for now.`);
    }
    return items[0];
}
function getConnectionSettings(program, itemModel) {
    return (program.stateMap(COCOGEN_STATE_CONNECTION_SETTINGS).get(itemModel) ?? {});
}
function getSourcePathSyntax(inputFormat) {
    return inputFormat === "csv" ? "csv" : "jsonpath";
}
function getProfileSourceSettings(program, itemModel) {
    return program.stateMap(COCOGEN_STATE_PROFILE_SOURCE_SETTINGS).get(itemModel);
}
function computeGraphApiVersion(contentCategory, usesPrincipal) {
...
function getStringArray(program, key, prop) {
    const raw = program.stateMap(key).get(prop);
    return raw ? [...raw] : [];
}
function getSearchFlags(program, prop) {
    const raw = (program.stateMap(COCOGEN_STATE_PROPERTY_SEARCH).get(prop) ?? {});
    return { ...raw };
}
function normalizeSourceSettings(raw, fallbackName, explicit, noSource, sourcePathSyntax, defaultValue, transformsValue) {
    const settings = (raw ?? {});
    ...
    const supportedTransforms = new Set(["trim", "lowercase", "uppercase"]);
    const normalizedTransforms = rawTransforms?.map((transform) => {
        if (typeof transform !== "string" || !supportedTransforms.has(transform)) {
            throw new CocogenError(`@coco.source transform '${String(transform)}' is not supported. Supported transforms: trim, lowercase, uppercase.`);
        }
        return transform;
    });
    ...
    if (sourcePathSyntax === "jsonpath") {
        ...
        if (normalized)
            assertValidJsonPath(normalized, (message) => new CocogenError(message));
        return {
            csvHeaders: [],
            jsonPath: normalized,
            explicit,
    ...
    if (typeof csv === "string") {
        ...
        throw new CocogenError("Source field merging is not supported. Use a single CSV header per property or preprocess your data.");
    }
...
function getSourceSettings(program, prop, fallbackName, sourcePathSyntax) {
    const map = program.stateMap(COCOGEN_STATE_PROPERTY_SOURCE);
    const raw = map.get(prop);
    const explicit = map.has(prop);
    const defaultOverride = program.stateMap(COCOGEN_STATE_PROPERTY_SOURCE_DEFAULT).get(prop);
    const transformsOverride = program.stateMap(COCOGEN_STATE_PROPERTY_SOURCE_TRANSFORMS).get(prop);
...
function getPersonEntityMapping(program, prop, propertyType, sourcePathSyntax) {
...
function getMappedObjectMapping(program, prop, propertyType, sourcePathSyntax) {
...
function collectStructuredMappingFields(program, prop, sourcePathSyntax, entity) {
    const rawFields = program.stateMap(COCOGEN_STATE_PROPERTY_PERSON_FIELDS).get(prop) ?? [];
    const extractSourcePath = (source) => {
        if (typeof source === "string")
            return source.trim();
        const jsonPath = typeof source.jsonPath === "string" ? source.jsonPath.trim() : "";
        return jsonPath.length > 0 ? jsonPath : undefined;
    };
    const normalizePrefixCandidate = (raw) => {
        let text = raw.trim();
        if (!text)
            return undefined;
        if (text.startsWith("$."))
    ...
    const applyCommonPrefix = (source) => {
        if (!commonPrefix || sourcePathSyntax !== "jsonpath")
            return source;
    ...
    const normalizeEntityPath = (rawPath) => {
        const trimmed = rawPath.trim();
        if (!trimmed)
            return "";
    ...
    const fields = rawFields
        ...
        if (!path)
            return undefined;
        const sourceSettings = normalizeSourceSettings(applyCommonPrefix(field.source), path, true, false, sourcePathSyntax, field.default, field.transforms);
        return {
            path,
            source: {
...
function hasStructuredMappingEntries(program, prop) {
    const rawFields = program.stateMap(COCOGEN_STATE_PROPERTY_PERSON_FIELDS).get(prop);
    return Boolean(rawFields && rawFields.length > 0);
}
function getSerializedModel(program, prop) {
    const raw = program.stateMap(COCOGEN_STATE_PROPERTY_SERIALIZED).get(prop);
    if (!raw)
        return undefined;
    let model = raw;
...
function getTypeSpecDescription(program, prop) {
    const fromDecorator = program.stateMap(COCOGEN_STATE_PROPERTY_TYPESPEC_DESCRIPTIONS).get(prop);
    if (fromDecorator && fromDecorator.trim().length > 0)
    ...
    for (const decorator of prop.decorators) {
        ...
        if (!definition || definition.name !== "@description")
            continue;
        if (definition.namespace?.name === "coco")
            continue;
...
function getSchemaDescription(program, prop) {
    const fromSchemaDecorator = program.stateMap(COCOGEN_STATE_PROPERTY_SCHEMA_DESCRIPTIONS).get(prop);
    if (fromSchemaDecorator && fromSchemaDecorator.trim().length > 0) {
        return { text: fromSchemaDecorator, source: "coco.schemaDescription" };
    }
    const fromLegacyDecorator = program.stateMap(COCOGEN_STATE_PROPERTY_LEGACY_DESCRIPTIONS).get(prop);
...
function getPattern(program, prop) {
...
function getName(program, prop) {
    const override = program.stateMap(COCOGEN_STATE_PROPERTY_NAME_OVERRIDES).get(prop);
    return override ?? prop.name;
}
function isContentProperty(program, prop) {
    return Boolean(program.stateMap(COCOGEN_STATE_CONTENT_PROPERTIES).get(prop));
}
function getContentSettings(program, prop) {
    return program.stateMap(COCOGEN_STATE_CONTENT_SETTINGS).get(prop);
}
function getContentSources(program, prop, sourcePathSyntax) {
    const rawEntries = program.stateMap(COCOGEN_STATE_PROPERTY_SOURCE_ENTRIES).get(prop);
    if (!rawEntries || rawEntries.length === 0)
        return [];
    const normalizeLabel = (entry) => {
        if (entry.to && entry.to.trim().length > 0)
            return entry.to.trim();
        if (typeof entry.from === "string")
            return entry.from.trim();
        if (entry.from && typeof entry.from === "object") {
            if (typeof entry.from.csv === "string")
                return entry.from.csv.trim();
            if (typeof entry.from.jsonPath === "string")
                return entry.from.jsonPath.trim();
        }
        return "";
    };
    return rawEntries
...
function isIdProperty(program, prop) {
    return Boolean(program.stateMap(COCOGEN_STATE_ID_PROPERTIES).get(prop));
}
function getIdEncoding(program, prop) {
    const settings = program.stateMap(COCOGEN_STATE_ID_SETTINGS).get(prop);
    const encoding = settings?.encoding;
    if (encoding === "base64" || encoding === "hash" || encoding === "slug")
        return encoding;
    return "slug";
}
function isPrincipalScalar(type) {
    if (type.kind !== "Scalar")
        return false;
    const scalar = type;
    return scalar.name === "Principal" && scalar.namespace?.name === "coco";
}
function mapTypeToPropertyType(program, type) {
    // Arrays
            ...
            default:
                {
                    ...
                    throw new CocogenError(`Unsupported TypeSpec scalar type: ${scalarName}. Supported scalars: string, boolean, int64, float64, utcDateTime, coco.Principal.${hint}`);
                }
        }
    }
    if (type.kind === "Enum") {
    ...
    if (type.kind === "Model") {
        throw new CocogenError("Nested models are not supported for connector schema properties. Flatten the model so every property is a scalar or scalar collection.");
    }
    throw new CocogenError(`Unsupported TypeSpec property type kind: ${type.kind}`);
}
function mapCollectionType(element) {
    if (element.kind === "Scalar") {
        const scalarName = element.name;
        switch (scalarName) {
            case "string":
...
export async function compileTypeSpec(entryTspPath) {
...
export async function loadIrFromTypeSpec(entryTspPath, options = {}) {
    const program = await compileTypeSpec(entryTspPath);
    if (program.hasError()) {
        const missingConnectionSettings = program.diagnostics.some((d) => d.severity === "error" && d.code === "invalid-argument" && String(d.message).includes("coco.ConnectionSettings"));
    ...
    if (idProps.length !== 1) {
        throw new CocogenError(idProps.length === 0
    ...
    const irProperties = properties
        ...
        if (hasStructuredMappings && !personEntity && !mappedObject && propertyType !== "principal" && propertyType !== "principalCollection") {
            throw new CocogenError(`Property '${prop.name}' uses @coco.source(..., to) mappings on unsupported type '${propertyType}'. Anonymous structured mappings are only supported for string or string[] properties; add a people label for strongly typed people entities.`);
        }
        if (serialized) {
    ...
    const connectionDescription = typeof connection.connectionDescription === "string" ? connection.connectionDescription.trim() : "";
    const provisionalIr = {
        connection: {
    ...
    const graphOperationVersions = Object.fromEntries(getGraphOperationRequirements(provisionalIr).map((requirement) => [
