import { getBlockedPeopleLabel, getPeopleLabelDefinition, getPeopleLabelInfo, isSupportedPeopleLabel } from "../people/label-registry.js";
...
function matchesRequiredField(path, fieldName) {
    if (!path)
        return false;
    return path === fieldName || path.startsWith(`${fieldName}.`);
}
function resolvePersonEntityPathIssue(entityName, path) {
    if (!path)
        return null;
...
const SEMANTIC_LABEL_TYPE_RULES = new Map([
    ["title", ["string"]],
    ["url", ["string"]],
    ["createdBy", ["string"]],
            ...
            for (const label of prop.labels) {
                ...
                if (!prop.personEntity) {
                    if (prop.source?.noSource) {
                        ...
                        issues.push({
                            severity: "error",
                            message: `People label '${label}' on property '${prop.name}' must map required Graph field(s) ${requiredFields.join(", ")}, but no @coco.source(..., to) entries were found.`,
                ...
                if (requiredFields.length > 0) {
                    const providedPaths = prop.personEntity.fields?.map((field) => field.path).filter((path) => Boolean(path)) ?? [];
                    const missingFields = requiredFields.filter((required) => !providedPaths.some((path) => matchesRequiredField(path, required)));
