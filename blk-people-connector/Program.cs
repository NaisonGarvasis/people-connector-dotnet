...
void PrintBanner()
{
    var title = "✨ 🥥 BLKPersonProfile connector CLI 🥥 ✨";
...
async Task ProvisionAsync()
{
    ...
    var graph = CreateGraphClient(credential);
...
async Task IngestAsync(string? inputPath, bool dryRun, int? limit, int? batchSize, int? retryAttempts, bool verbose, bool failFast)
{
    ...
    if (!dryRun)
    {
        ...
        graph = CreateGraphClient(credential);
    ...
    IItemSource<ItemModel> source = new JsonItemSource(path);
    var core = BuildConnectorCore(graph, credential, connectionId);
...
provisionCommand.SetAction(async (parseResult, cancellationToken) =>
